            ...
            <div class="box-content">
                ...
                <table class="table table-advance" id="table_quiz_type">
                    ...
                    @foreach ($QuizTypeList as $item)
                        ...
                        <tr>
                            <td class="Id"><a href="<?php echo url()."/quiztype/detail/".$item->getQuizType(); ?>">{{ $item->getQuizType() }}</a></td>
                            <td class="Name">{{ $item->getName() }}</td>
                            <td>
                                <a class='btn btn-small show-tooltip' title='Edit' href='<?php echo url()."/quiztype/edit/".$item->getQuizType(); ?>'><i class='icon-edit'></i></a>
                                <a class='btn btn-small btn-danger show-tooltip' id="btnDelete" title='Delete' href='<?php echo url()."/quiztype/delete/".$item->getQuizType(); ?>'><i class='icon-trash'></i></a>
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
